...
class Network:
    ...
    def SGD_train(self, train, epochs, eta, lam=0.0, verbose=True, test=None):
        ...
        for epoch in range(epochs):
            ...
            for kk in range(n_train):
                ...
                self.weights = [ (1-eta*lam)*w for w in self.weights]
    ...
    def back_prop(self, x, y):
        ...
        delta  = np.multiply((a_list[self.L-1]-y),sigmoid_prime(z_list[self.L-1]))
        for ell in range(self.L-2,-1,-1):
             db_list[ell] = delta
             dW_list[ell] = np.matmul(delta,a_list[ell].transpose())
             delta = np.multiply(np.matmul(self.weights[ell].transpose(), delta), sigmoid_prime(z_list[ell]))
...
if __name__ == "__main__":
    ...
    nn.SGD_train(train, epochs=1, eta=0.25, lam=0.0, verbose=True, test=test)
