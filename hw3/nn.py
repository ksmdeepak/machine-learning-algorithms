...
class Network:
    ...
    def SGD_train(self, train, epochs, eta, lam=0.0, verbose=True, test=None):
        ...
        for epoch in range(epochs):
            ...
            for kk in range(n_train):
                ...
                self.biases = [b - eta*(db + lam*b) for (b, db) in zip(self.biases, dbs)]
            if verbose:
    ...
    def back_prop(self, x, y):
        ...
        delta = np.multiply(a_list[len(a_list)-1]-y,self.g_prime(z_list[len(z_list)-1]))
        # TODO delta  =
        for ell in range(self.L-2,-1,-1):
            db_list[ell] = delta
            dW_list[ell] = np.dot(delta,np.transpose(a_list[ell]))
            delta =  np.multiply(np.dot(np.transpose(self.weights[ell]),delta),self.g_prime(z_list[ell]))
            # TODO db_list[ell] =
            # TODO dW_list[ell] =
            # TODO delta =
            pass
