...
class LogReg:
    ...
    def sgd_update(self, x_i, y):
        ...
        for (count,feature) in enumerate(self.w):
            self.w[count]= self.w[count]+ self.eta*(y-pi)*x_i[count]
...
def sigmoid(score, threshold=20.0):
    ...
    score =  np.exp(score)/(np.exp(score)+1)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.passes):
        ...
        for (x,y) in zip(data.train_x,data.train_y):
            # if(count%200==0):
            #     print(count)
            #     value = lr.progress(data.test_x,data.test_y)
            #     print(value)
            lr.sgd_update(x,y)
            count+=1
        print(iteration)
